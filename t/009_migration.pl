# Check large object migration between native and pg_lolor storage
#
...
# preserve, then move them into pg_lolor storage and back.
#
# ##############################################################################

$node->safe_psql('postgres', "SELECT lolor.disable()");
$node->safe_psql('postgres', "CREATE ROLE regress_lolor_owner");
$node->safe_psql('postgres', "CREATE ROLE regress_lolor_grantee");

my $plain = $node->safe_psql('postgres',
	"SELECT lo_from_bytea(0, 'annotated native object')");
$node->safe_psql(
	'postgres', qq(
	ALTER LARGE OBJECT $plain OWNER TO regress_lolor_owner;
	GRANT SELECT ON LARGE OBJECT $plain TO regress_lolor_grantee;
...
is( $node->safe_psql(
	...
	'0',
	"native storage emptied by migrate_from_native()");

is( $node->safe_psql('postgres', "SELECT count(*) FROM lolor.pg_largeobject"),
	$native_pages,
...
is( $node->safe_psql(
	...
	'0',
	"native shared dependencies removed with the objects");
...
is( $node->safe_psql(
	...
	"comment parked while in pg_lolor storage");
...
is( $node->safe_psql(
	...
	'regress_lolor_owner',
	"owner preserved into pg_lolor storage");

is( $node->safe_psql(
		'postgres', "SELECT convert_from(lo_get($plain), 'UTF8')"),
	'annotated native object',
	"content readable from pg_lolor storage");
...
is( $node->safe_psql(
		'postgres', "SELECT count(*) FROM lolor.pg_largeobject_metadata"),
	'0',
	"pg_lolor storage emptied by migrate_to_native()");
...
is( $node->safe_psql(
	...
	'a:regress_lolor_grantee,o:regress_lolor_owner',
...
is( $node->safe_psql(
	...
	'survives the round trip',
	"comment reinstated on the catalog object");

is( $node->safe_psql(
		'postgres', "SELECT count(*) FROM lolor.pg_largeobject_description"),
	'0',
	"comment parking table drained");

is( $node->safe_psql(
		'postgres', "SELECT count(*) FROM pg_catalog.pg_largeobject"),
	$native_pages,
	"page count still exact after the return trip");

# DROP ROLE must now see the objects, which a raw catalog update would not
# have allowed.
($result, $stdout, $stderr) =
  $node->psql('postgres', "DROP ROLE regress_lolor_owner");
like(
	$stderr,
	qr/cannot be dropped because some objects depend on it/,
	"DROP ROLE refuses while the role owns migrated large objects");

($result, $stdout, $stderr) =
  $node->psql('postgres', "DROP ROLE regress_lolor_grantee");
like(
	$stderr,
	qr/cannot be dropped because some objects depend on it/,
...
is( $node->safe_psql(
		'postgres', "SELECT convert_from(lo_get($plain), 'UTF8')"),
	'annotated native object',
	"content survives a clean restart");
...
is( $node->safe_psql(
		'postgres', "SELECT convert_from(lo_get($plain), 'UTF8')"),
	'annotated native object',
	"content survives an immediate stop and recovery");

is( $node->safe_psql(
		'postgres', "SELECT convert_from(lo_get($crash_oid), 'UTF8')"),
	'written before a crash',
	"a committed object written just before the crash is recovered");

# ##############################################################################
#
# There is no way to keep the migration out of logical decoding,
...
$node->safe_psql('postgres', "SELECT lolor.migrate_to_native()");
$node->safe_psql('postgres',
	"SELECT pg_create_logical_replication_slot('pg_lolor_test_slot', 'pgoutput')"
);
...
like(
	$stderr,
	qr/while logical replication slots exist/,
...
is( $node->safe_psql(
	...
	't',
	"refusal left the native objects untouched");

# The zero-object case returns early, so put something in pg_lolor storage for
# the guard to actually be reached.
$node->safe_psql('postgres',
	"SELECT lo_from_bytea(0, 'held in pg_lolor storage')");

($result, $stdout, $stderr) =
  $node->psql('postgres', "SELECT lolor.migrate_to_native()");
isnt($result, 0,
	"migrate_to_native() raises an error rather than reporting -1");
like(
	$stderr,
	qr/cannot migrate large objects/,
	"migrate_to_native() refuses on the drop path rather than losing objects"
);

$node->safe_psql('postgres',
	"SELECT pg_drop_replication_slot('pg_lolor_test_slot')");
