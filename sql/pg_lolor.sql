...
DROP EXTENSION pg_lolor;

-- Write and read back through a fresh install
CREATE EXTENSION pg_lolor;
SELECT lo_creat(-1) AS loid \gset
BEGIN;
SELECT lo_open(:loid, x'60000'::int) AS fd \gset
...
SELECT convert_from(loread(:fd, 1024), 'UTF8');
END;
-- Verify the migration functions
...
SELECT lo_from_bytea(1, 'Example large object stored in pg_lolor LO storage');
...
-- DROP EXTENSION while pg_lolor is disabled.  The reverse migration works
...
SELECT lolor.disable();
DROP EXTENSION pg_lolor;
SELECT extname FROM pg_extension; -- check pg_lolor removal
-- The object was migrated to native storage, not dropped with pg_lolor's tables
...
-- Install pg_lolor and migrate native LOs into pg_lolor storage
CREATE EXTENSION pg_lolor;
SELECT lolor.migrate_from_native();
...
SELECT count(*) AS pg_lolor_after_migrate FROM lolor.pg_largeobject_metadata;
...
-- Create an additional LO directly in pg_lolor storage
SELECT lo_from_bytea(0, 'Created directly in pg_lolor') AS pg_lolor_direct_oid \gset
...
-- After DROP: expect "Created directly in pg_lolor"
SELECT convert_from(lo_get(:'pg_lolor_direct_oid'::oid), 'UTF8') AS obj3_after_reverse;

-- Cleanup native LOs
SELECT lo_unlink(:'native_oid1'::oid);
SELECT lo_unlink(:'native_oid2'::oid);
SELECT lo_unlink(:'pg_lolor_direct_oid'::oid);
...
SELECT count(*) AS pg_lolor_after_manual FROM lolor.pg_largeobject_metadata;

-- After manual migration: expect "Manual reverse test"
BEGIN;
-- Disable pg_lolor to read from native storage directly
SELECT lolor.disable();
...
CREATE EXTENSION pg_lolor;
-- HACK: Manually insert a row with the same OID into pg_lolor storage
...
-- Disable pg_lolor to create a native LO with the same OID
SELECT lolor.disable();
...
SELECT extname FROM pg_extension WHERE extname = 'pg_lolor';
...
-- revokes EXECUTE on them from PUBLIC.  pg_lolor replaces them by renaming the
...
--
-- pg_lolor.node is bounded by the OID encoding, not by an independently written
-- constant: the low PG_LOLOR_NODEID_BITS of a generated OID carry the node id, so
-- 16 does not fit and was previously accepted while encoding as node 0.
--
SET pg_lolor.node = 16;
SET pg_lolor.node = 15;
SET pg_lolor.node = 1;

--
-- Permission enforcement.
--
-- Objects in pg_lolor storage have no catalog entry, so pg_lolor cannot use the
...
--
CREATE EXTENSION pg_lolor;
CREATE ROLE regress_lolor_alice;
CREATE ROLE regress_lolor_bob;
...
CREATE FUNCTION regress_lolor_expect_error(cmd text) RETURNS text AS $$
BEGIN
  EXECUTE cmd;
...
SET ROLE regress_lolor_bob;
SELECT regress_lolor_expect_error(format('SELECT lo_get(%s)', :alice_oid)) AS read_denied;
SELECT regress_lolor_expect_error(format('SELECT lo_open(%s, 262144)', :alice_oid)) AS open_denied;
SELECT regress_lolor_expect_error(format('SELECT lo_put(%s, 0, ''x'')', :alice_oid)) AS write_denied;
SELECT regress_lolor_expect_error(format('SELECT lo_unlink(%s)', :alice_oid)) AS unlink_denied;
RESET ROLE;
...
-- object in pg_lolor storage has no row.  This limitation is documented; assert
-- it so that a change in behaviour is noticed.
SELECT regress_lolor_expect_error(
  format('GRANT SELECT ON LARGE OBJECT %s TO regress_lolor_bob', :alice_oid)) AS grant_unsupported;
SELECT regress_lolor_expect_error(
  format('ALTER LARGE OBJECT %s OWNER TO regress_lolor_bob', :alice_oid)) AS alter_unsupported;

SELECT lo_unlink(:alice_oid);

--
-- Objects in pg_lolor storage are rows in ordinary tables and cannot participate
...
DROP ROLE regress_lolor_bob;
DROP FUNCTION regress_lolor_expect_error(text);
DROP EXTENSION pg_lolor;
...
--
-- A rolled back transaction must leave no trace in pg_lolor storage.
--
...
-- size exercises the page-assembly path in pg_lolor_inv_read().
--
...
--
-- An unprivileged user must not be able to wedge pg_lolor by squatting the
-- names the state probes look for.  Matching on proname alone would make
-- is_enabled() raise "inconsistent state" and block DROP EXTENSION.
--
CREATE EXTENSION pg_lolor;
CREATE ROLE regress_lolor_squatter;
GRANT CREATE ON SCHEMA public TO regress_lolor_squatter;
SET ROLE regress_lolor_squatter;
CREATE FUNCTION public.pg_lolor_lo_open(oid, int4) RETURNS int4
  AS 'SELECT 1' LANGUAGE sql;
CREATE FUNCTION public.lo_close_orig(int4) RETURNS int4
  AS 'SELECT 1' LANGUAGE sql;
RESET ROLE;
SELECT lolor.is_enabled() AS unaffected_by_squatting;
DROP FUNCTION public.pg_lolor_lo_open(oid, int4);
DROP FUNCTION public.lo_close_orig(int4);
REVOKE CREATE ON SCHEMA public FROM regress_lolor_squatter;
DROP ROLE regress_lolor_squatter;
DROP EXTENSION pg_lolor;
...
SELECT count(*) AS ext_left FROM pg_extension WHERE extname = 'pg_lolor';
...
SELECT count(*) AS extension_still_installed
  FROM pg_extension WHERE extname = 'pg_lolor';
...
--
CREATE EXTENSION pg_lolor;
CREATE ROLE regress_lolor_owner;
CREATE ROLE regress_lolor_grantee;
...
ALTER LARGE OBJECT :annotated_oid OWNER TO regress_lolor_owner;
GRANT SELECT ON LARGE OBJECT :annotated_oid TO regress_lolor_grantee;
...
DROP ROLE regress_lolor_owner;
DROP ROLE regress_lolor_grantee;
...
CREATE ROLE regress_lolor_nosuper;
GRANT USAGE ON SCHEMA lolor TO regress_lolor_nosuper;
SET ROLE regress_lolor_nosuper;
SELECT lolor.migrate_storage(true);
SELECT lolor.migrate_from_native();
SELECT lolor.migrate_to_native();
RESET ROLE;
REVOKE USAGE ON SCHEMA lolor FROM regress_lolor_nosuper;
DROP ROLE regress_lolor_nosuper;
...
-- Security labels cannot be represented in pg_lolor storage and cannot be
...
VALUES (:peer_oid, 'pg_catalog.pg_largeobject'::regclass, 0, 'pg_lolor_test', 'secret');
SELECT lolor.migrate_from_native();
DELETE FROM pg_catalog.pg_seclabel
 WHERE classoid = 'pg_catalog.pg_largeobject'::regclass AND provider = 'pg_lolor_test';
