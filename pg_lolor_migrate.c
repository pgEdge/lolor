 ...
 *	  (pg_catalog.pg_largeobject{,_metadata}) and pg_lolor's replicated user
 *	  tables (lolor.pg_largeobject{,_metadata}).
 *
 * pg_lolor's tables have the same column layout as the catalogs they stand in
 * for (see pg_lolor--1.0.sql), which makes migration a plain tuple copy rather
 * than a decode/re-encode through the large object API.  The layout is
 * verified at run time rather than assumed.
 *
 * The rest is the bookkeeping only the real catalogs take part in: pg_shdepend
 * rows for the owner and ACL grantees, and pg_description comments, which have
 * nowhere to live while an object sits in pg_lolor storage and are parked in
 * lolor.pg_largeobject_description for the duration.  Native objects are
 * removed through performMultipleDeletions(), the path DROP uses, so core
 * cleans up dependencies, comments and security labels.
 *
 * Copyright (c) 2022-2026, pgEdge, Inc.
 * Portions Copyright (c) 1996-2025, PostgreSQL Global Development Group
 *
 * IDENTIFICATION
 *	  contrib/pg_lolor/pg_lolor_migrate.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(pg_lolor_migrate_storage);
...
{
	bool		to_native;		/* pg_lolor -> pg_catalog when true */
 ...
 *
 * A mismatch means the extension was built against a different catalog
 * definition than the server runs, and copying tuples would corrupt data.
 */
static void
			...
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					 ...
					 errdetail("Column %d differs between \"%s\" and \"%s\".",
							   i + 1, RelationGetRelationName(a),
							   RelationGetRelationName(b)),
					 errhint("The pg_lolor extension was built against a different "
 ...
 *
 * heap_form_tuple() copies the pointer it is handed, so an external datum
 * belonging to the source's TOAST table would reach the destination as a
 * dangling reference once the source row is removed.  lolor.pg_largeobject has
 * a TOAST table of its own and a full page of incompressible data can be
 * pushed out of line, so this is reachable in practice.
 *
 * detoast_attr() returns a plain varlena untouched, so this is cheap.
 */
static void
flatten_varlenas(TupleDesc desc, Datum *values, const bool *nulls)
{
	...
	for (i = 0; i < desc->natts; i++)
	{
		...
		va = (struct varlena *) DatumGetPointer(values[i]);
		values[i] = PointerGetDatum(detoast_attr(va));
	}
}
...
check_oid_conflicts(MigrateRels *rels)
{
	...
	if (nconflicts > MIGRATE_MAX_REPORTED_CONFLICTS)
		appendStringInfo(&buf, " and %lld more",
						 (long long) (nconflicts - MIGRATE_MAX_REPORTED_CONFLICTS));
	...
	ereport(ERROR,
			(errcode(ERRCODE_DUPLICATE_OBJECT),
			 ...
			 errhint("Nothing has been migrated. "
					 "Remove or rename the conflicting large objects and retry.")));
}
...
copy_metadata(MigrateRels *rels)
{
	...
	/*
	 * Each iteration allocates a detoasted datum, the formed tuple, index
	 * scratch and any comment text.  CurrentMemoryContext is not reset within
	 * a function call, so on a database with millions of large objects those
	 * would accumulate until the backend ran out of memory, aborting an
	 * all-or-nothing migration.  Reset a per-row context instead.
	 */
	tmpcxt = AllocSetContextCreate(CurrentMemoryContext,
								   "pg_lolor migrate metadata",
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		if (rels->to_native)
		{
			...
			/* Restore any comment parked on the way into pg_lolor storage. */
			{
		...
		{
			/*
			 * pg_lolor's tables are ordinary tables, so a comment has nowhere
			 * to attach while the object lives there.  Park it so the round
			 * trip back to native storage is lossless.  The pg_description
			 * row itself is removed by performMultipleDeletions() later.
			 */
...
/*
 * Remove the native large objects that have just been copied into pg_lolor.
 *
...
/*
 * Delete every row of an ordinary pg_lolor storage table.
 */
static void
 ...
 */
Datum
pg_lolor_migrate_storage(PG_FUNCTION_ARGS)
{
	bool		to_native = PG_GETARG_BOOL(0);
	MigrateRels rels;
	Relation	native_meta;
	Relation	native_data;
	Relation	pg_lolor_meta;
	Relation	pg_lolor_data;
	Oid			pg_lolor_meta_id;
	Oid			pg_lolor_data_id;
	int64		nobjects;
	int64		npages;
	...
	pg_lolor_meta_id = get_PG_LOLOR_LargeObjectMetadataRelationId();
	pg_lolor_data_id = get_PG_LOLOR_LargeObjectRelationId();
	 ...
	 * conflict with itself, so a concurrent lo_write() could commit between
	 * the page copy and the emptying of the source store and be lost without
	 * trace.  ShareRowExclusiveLock conflicts with both, so writers wait and
	 * two migrations serialise.  The locks are taken in the same order in
	 * either direction, so opposing migrations cannot deadlock.
	 */
	...
	pg_lolor_meta = table_open(pg_lolor_meta_id, ShareRowExclusiveLock);
	pg_lolor_data = table_open(pg_lolor_data_id, ShareRowExclusiveLock);

	rels.desc_rel = table_open(get_PG_LOLOR_LargeObjectDescriptionRelationId(),
							   ShareRowExclusiveLock);
	rels.desc_idx = get_PG_LOLOR_LargeObjectDescriptionIndexId();

	if (to_native)
	{
		rels.src_meta = pg_lolor_meta;
		rels.src_data = pg_lolor_data;
		rels.dst_meta = native_meta;
		rels.dst_data = native_data;

		rels.src_meta_idx = get_PG_LOLOR_LargeObjectMetadataOidIndexId();
		rels.src_data_idx = get_PG_LOLOR_LargeObjectLOidPNIndexId();
	...
	else
	{
		...
		rels.dst_meta = pg_lolor_meta;
		rels.dst_data = pg_lolor_data;

		rels.src_meta_idx = LargeObjectMetadataOidIndexId;
		rels.src_data_idx = LargeObjectLOidPNIndexId;
		rels.dst_meta_idx = get_PG_LOLOR_LargeObjectMetadataOidIndexId();
		rels.dst_data_idx = get_PG_LOLOR_LargeObjectLOidPNIndexId();
	}
	...
	/*
	 * Close the relcache references but hold the locks until the caller's
	 * transaction ends.  Releasing them here would reopen the window this
	 * function exists to close: a write landing between the release and the
	 * commit would vanish when the emptied source store became visible.
	 */
	table_close(rels.desc_rel, NoLock);
	table_close(pg_lolor_data, NoLock);
	table_close(pg_lolor_meta, NoLock);
	table_close(native_data, NoLock);
	table_close(native_meta, NoLock);

	ereport(NOTICE,
			(errmsg_plural("migrated %lld large object to %s storage",
						   "migrated %lld large objects to %s storage",
						   (unsigned long) nobjects,
						   (long long) nobjects,
						   to_native ? "native" : "pg_lolor"),
			 errdetail_plural("%lld data page copied.",
							  "%lld data pages copied.",
