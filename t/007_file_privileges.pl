...
$node->safe_psql('postgres', "CREATE ROLE regress_lolor_plain LOGIN");

# lo_import() and lo_export() read and write files as the server's operating
# system account.  Core revokes EXECUTE on them from PUBLIC; pg_lolor's
...
my $srcfile = "$filedir/pg_lolor_import_source.txt";
...
($result, $stdout, $stderr) = $node->psql(
	...
	extra_params => [ '-U', 'regress_lolor_plain' ]);
like(
	$stderr,
	qr/permission denied for function lo_import/,
	"lo_import() is not executable by an ordinary user");

my $dstfile = "$filedir/pg_lolor_export_target.txt";
($result, $stdout, $stderr) = $node->psql(
	'postgres',
	"SELECT lo_export(1, '$dstfile')",
	extra_params => [ '-U', 'regress_lolor_plain' ]);
like(
	$stderr,
	qr/permission denied for function lo_export/,
	"lo_export() is not executable by an ordinary user");

ok(!-e $dstfile, "the refused lo_export() wrote no file");

# The superuser can still round-trip a file through pg_lolor storage.
my $imported = $node->safe_psql('postgres', "SELECT lo_import('$srcfile')");
is( $node->safe_psql(
		'postgres', "SELECT convert_from(lo_get($imported), 'UTF8')"),
	"imported through a large object\n",
	"lo_import() stores the file contents in pg_lolor storage");

$node->safe_psql('postgres', "SELECT lo_export($imported, '$dstfile')");
ok(-e $dstfile, "lo_export() wrote the file");
is( slurp_file($dstfile),
	"imported through a large object\n",
