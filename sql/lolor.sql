...
ALTER EXTENSION lolor UPDATE TO '1.4.0';
...
-- migration ran through the renamed _orig functions.  It now works against the
-- catalogs directly, so the disabled state is no longer a special case and the
-- objects are still rescued.
CREATE EXTENSION lolor;
...
-- created with the default unless locked down explicitly.  Before 1.4.0 that
-- let any database user read or overwrite server files.
--
CREATE EXTENSION lolor;
