 ...
 *
 * lolor's tables have the same column layout as the catalogs they stand in
 * for (see lolor--1.0.sql), which makes migration a plain tuple copy rather
 ...
 * nowhere to live while an object sits in lolor storage and are parked in
...
/*
 * Pre-flight role check for migration to native storage.
 *
 * Recording ownership and ACLs in pg_shdepend goes through
 * shdepLockAndCheckObject(), which reports a missing role as "role %u was
 * concurrently dropped".  Nothing was dropped concurrently: objects in lolor
 * storage cannot take part in pg_shdepend, so DROP ROLE does not see them and
 * an owner, grantee or grantor can vanish while they sit there.  Find those
 * first and say what happened; otherwise one such object blocks the whole
 * migration, and with it DROP EXTENSION.
 */
static void
check_orphaned_roles(MigrateRels *rels)
{
	TupleDesc	desc = RelationGetDescr(rels->src_meta);
	SysScanDesc scan;
	HeapTuple	tup;
	StringInfoData buf;
	int64		norphans = 0;
	...
	/* Detoasting the ACL and aclmembers() allocate; reset per row. */
	tmpcxt = AllocSetContextCreate(CurrentMemoryContext,
								   "lolor migrate role check",
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		orphan = !SearchSysCacheExists1(AUTHOID, ObjectIdGetDatum(owner));

		if (!orphan && !nulls[Anum_pg_largeobject_metadata_lomacl - 1])
		{
			Acl		   *acl = DatumGetAclP(values[Anum_pg_largeobject_metadata_lomacl - 1]);
			Oid		   *roles;
			int			nroles = aclmembers(acl, &roles);
			int			i;

			/* aclmembers() covers grantors and grantees and skips PUBLIC. */
			for (i = 0; i < nroles; i++)
			{
				if (!SearchSysCacheExists1(AUTHOID, ObjectIdGetDatum(roles[i])))
				{
					orphan = true;
					break;
				}
		...
		if (norphans < MIGRATE_MAX_REPORTED_CONFLICTS)
			appendStringInfo(&buf, "%s%u", norphans > 0 ? ", " : "", loid);
		norphans++;
	}
	systable_endscan(scan);
	...
	if (norphans > MIGRATE_MAX_REPORTED_CONFLICTS)
		appendStringInfo(&buf, " and %lld more",
						 (long long) (norphans - MIGRATE_MAX_REPORTED_CONFLICTS));

	ereport(ERROR,
			(errcode(ERRCODE_UNDEFINED_OBJECT),
			 errmsg_plural("%lld large object refers to a role that no longer exists",
						   "%lld large objects refer to roles that no longer exist",
						   (unsigned long) norphans,
						   (long long) norphans),
			 errdetail("Affected OID(s): %s.", buf.data),
			 errhint("Nothing has been migrated. List them with lolor.check_orphans(), "
					 "repair them with lolor.fix_orphans(), then retry.")));
}
...
lolor_migrate_storage(PG_FUNCTION_ARGS)
{
	...
	/* Likewise if a role the catalogs must record has been dropped. */
	if (to_native)
		check_orphaned_roles(&rels);
	...
	ereport(NOTICE,
			(errmsg("migrated " INT64_FORMAT " large object(s), "
					INT64_FORMAT " data page(s), to %s storage",
