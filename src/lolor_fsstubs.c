 ...
 */
PG_FUNCTION_INFO_V1(lolor_lo_create);
PG_FUNCTION_INFO_V1(lolor_lo_import);
PG_FUNCTION_INFO_V1(lolor_lo_export);
PG_FUNCTION_INFO_V1(lolor_lo_import_with_oid);
PG_FUNCTION_INFO_V1(lolor_lo_open);
PG_FUNCTION_INFO_V1(lolor_lo_close);
PG_FUNCTION_INFO_V1(lolor_loread);
PG_FUNCTION_INFO_V1(lolor_lowrite);
PG_FUNCTION_INFO_V1(lolor_lo_lseek);
PG_FUNCTION_INFO_V1(lolor_lo_creat);
PG_FUNCTION_INFO_V1(lolor_lo_tell);
PG_FUNCTION_INFO_V1(lolor_lo_unlink);
PG_FUNCTION_INFO_V1(lolor_lo_truncate);
PG_FUNCTION_INFO_V1(lolor_lo_lseek64);
PG_FUNCTION_INFO_V1(lolor_lo_tell64);
PG_FUNCTION_INFO_V1(lolor_lo_truncate64);
PG_FUNCTION_INFO_V1(lolor_lo_from_bytea);
PG_FUNCTION_INFO_V1(lolor_lo_get);
PG_FUNCTION_INFO_V1(lolor_lo_get_fragment);
PG_FUNCTION_INFO_V1(lolor_lo_put);
...
lolor_lo_open(PG_FUNCTION_ARGS)
{
	...
	lobjDesc = lolor_inv_open(lobjId, mode, fscxt);
...
lo_read(int fd, char *buf, int len)
{
	...
	/*
	 * Check state.  lolor_inv_read() would throw an error anyway, but we want the
	...
	status = lolor_inv_read(lobj, buf, len);
...
lo_write(int fd, const char *buf, int len)
{
	...
	status = lolor_inv_write(lobj, buf, len);

	return status;
}

Datum
lolor_lo_lseek(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	...
	status = lolor_inv_seek(cookies[fd], offset, whence);
...
Datum
lolor_lo_lseek64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
...
Datum
lolor_lo_creat(PG_FUNCTION_ARGS)
{
	Oid			lobjId;
	...
	lobjId = lolor_inv_create(InvalidOid);

	PG_RETURN_OID(lobjId);
}

Datum
lolor_lo_create(PG_FUNCTION_ARGS)
{
	Oid			lobjId = PG_GETARG_OID(0);
	...
	lo_cleanup_needed = true;
	lobjId = lolor_inv_create(lobjId);

	PG_RETURN_OID(lobjId);
}

Datum
lolor_lo_tell(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int64		offset;
	...
	offset = lolor_inv_tell(cookies[fd]);
...
Datum
lolor_lo_tell64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int64		offset;
...
Datum
lolor_lowrite(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
 ...
 */
Datum
lolor_lo_import(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
lolor_lo_import_with_oid(PG_FUNCTION_ARGS)
{
	text	   *filename = PG_GETARG_TEXT_PP(0);
 ...
 */
Datum
lolor_lo_export(PG_FUNCTION_ARGS)
{
	Oid			lobjId = PG_GETARG_OID(0);
	 ...
	 */
	lo_cleanup_needed = true;
	lobj = lolor_inv_open(lobjId, INV_READ, CurrentMemoryContext);
	 ...
	 */
	text_to_cstring_buffer(filename, fnamebuf, sizeof(fnamebuf));
	oumask = umask(S_IWGRP | S_IWOTH);	/* nosemgrep */ /* Flawfinder: ignore — matches core be-fsstubs.c intentionally */
	PG_TRY();
	{
	...
	PG_FINALLY();
	{
		umask(oumask);	/* nosemgrep */ /* Flawfinder: ignore — restoring previous mask */
	}
	PG_END_TRY();
	if (fd < 0)
		ereport(ERROR,
	 ...
	 */
	while ((nbytes = lolor_inv_read(lobj, buf, BUFSIZE)) > 0)
	{
...
lo_truncate_internal(int32 fd, int64 len)
{
	...
	lolor_inv_truncate(lobj, len);
}

Datum
lolor_lo_truncate(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int32		len = PG_GETARG_INT32(1);
...
Datum
lolor_lo_truncate64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
 ...
 */
void
AtEOXact_LOLOR_LargeObject(bool isCommit)
{
	int			i;
 ...
 */
void
AtEOSubXact_LOLOR_LargeObject(bool isCommit, SubTransactionId mySubid,
...
lo_get_fragment_internal(Oid loOid, int64 offset, int32 nbytes)
{
	...
	lo_cleanup_needed = true;
	loDesc = lolor_inv_open(loOid, INV_READ, CurrentMemoryContext);

	/*
	 * Compute number of bytes we'll actually read, accommodating nbytes == -1
	 * and reads beyond the end of the LO.
	 */
	loSize = lolor_inv_seek(loDesc, 0, SEEK_END);
	if (loSize > offset)
	{
	...
	lolor_inv_seek(loDesc, offset, SEEK_SET);
	total_read = lolor_inv_read(loDesc, VARDATA(result), result_length);
 ...
 */
Datum
lolor_lo_get_fragment(PG_FUNCTION_ARGS)
{
	Oid			loOid = PG_GETARG_OID(0);
	int64		offset = PG_GETARG_INT64(1);
 ...
 */
Datum
lolor_lo_from_bytea(PG_FUNCTION_ARGS)
{
	Oid			loOid = PG_GETARG_OID(0);
	...
	lo_cleanup_needed = true;
	loOid = lolor_inv_create(loOid);
	loDesc = lolor_inv_open(loOid, INV_WRITE, CurrentMemoryContext);
	written = lolor_inv_write(loDesc, VARDATA_ANY(str), VARSIZE_ANY_EXHDR(str));
