 ...
 *
 * IDENTIFICATION
 *	  contrib/pg_lolor/pg_lolor_fsstubs.c
 *
 * NOTES
 ...
 */
PG_FUNCTION_INFO_V1(pg_lolor_lo_create);
PG_FUNCTION_INFO_V1(pg_lolor_lo_import);
PG_FUNCTION_INFO_V1(pg_lolor_lo_export);
PG_FUNCTION_INFO_V1(pg_lolor_lo_import_with_oid);
PG_FUNCTION_INFO_V1(pg_lolor_lo_open);
PG_FUNCTION_INFO_V1(pg_lolor_lo_close);
PG_FUNCTION_INFO_V1(pg_lolor_loread);
PG_FUNCTION_INFO_V1(pg_lolor_lowrite);
PG_FUNCTION_INFO_V1(pg_lolor_lo_lseek);
PG_FUNCTION_INFO_V1(pg_lolor_lo_creat);
PG_FUNCTION_INFO_V1(pg_lolor_lo_tell);
PG_FUNCTION_INFO_V1(pg_lolor_lo_unlink);
PG_FUNCTION_INFO_V1(pg_lolor_lo_truncate);
PG_FUNCTION_INFO_V1(pg_lolor_lo_lseek64);
PG_FUNCTION_INFO_V1(pg_lolor_lo_tell64);
PG_FUNCTION_INFO_V1(pg_lolor_lo_truncate64);
PG_FUNCTION_INFO_V1(pg_lolor_lo_from_bytea);
PG_FUNCTION_INFO_V1(pg_lolor_lo_get);
PG_FUNCTION_INFO_V1(pg_lolor_lo_get_fragment);
PG_FUNCTION_INFO_V1(pg_lolor_lo_put);

static int	lo_read(int fd, char *buf, int len);
static int	lo_write(int fd, const char *buf, int len);
static bool pg_lolor_object_ownercheck(Oid classid, Oid objectid, Oid roleid);
static AclMode pg_lolor_largeobject_aclmask_snapshot(Oid lobj_oid, Oid roleid,
...
Datum
pg_lolor_lo_open(PG_FUNCTION_ARGS)
{
	...
	lobjDesc = pg_lolor_inv_open(lobjId, mode, fscxt);
...
Datum
pg_lolor_lo_close(PG_FUNCTION_ARGS)
{
...
lo_read(int fd, char *buf, int len)
{
	...
	/*
	 * Check state.  pg_lolor_inv_read() would throw an error anyway, but we
	 * want the error to be about the FD's state not the underlying privilege;
	 * it might be that the privilege exists but user forgot to ask for read
	 * mode.
	 */
	...
	status = pg_lolor_inv_read(lobj, buf, len);
...
lo_write(int fd, const char *buf, int len)
{
	...
	status = pg_lolor_inv_write(lobj, buf, len);

	return status;
}

Datum
pg_lolor_lo_lseek(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	...
	status = pg_lolor_inv_seek(cookies[fd], offset, whence);
...
Datum
pg_lolor_lo_lseek64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
...
Datum
pg_lolor_lo_creat(PG_FUNCTION_ARGS)
{
	Oid			lobjId;
	...
	lobjId = pg_lolor_inv_create(InvalidOid);

	PG_RETURN_OID(lobjId);
}

Datum
pg_lolor_lo_create(PG_FUNCTION_ARGS)
{
	Oid			lobjId = PG_GETARG_OID(0);
	...
	lo_cleanup_needed = true;
	lobjId = pg_lolor_inv_create(lobjId);

	PG_RETURN_OID(lobjId);
}

Datum
pg_lolor_lo_tell(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int64		offset;
	...
	offset = pg_lolor_inv_tell(cookies[fd]);
...
Datum
pg_lolor_lo_tell64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int64		offset;
...
Datum
pg_lolor_lo_unlink(PG_FUNCTION_ARGS)
{
	Oid			lobjId = PG_GETARG_OID(0);
	 ...
	 * in pg_lolor_inv_drop(), but we want to throw the error before not after
	 * closing relevant FDs.
	 */
	if (!lo_compat_privileges &&
		!pg_lolor_object_ownercheck(get_PG_LOLOR_LargeObjectMetadataRelationId(),
									lobjId, GetUserId()))
		ereport(ERROR,
	...
	/*
	 * pg_lolor_inv_drop does not create a need for end-of-transaction cleanup
	 * and hence we don't need to set lo_cleanup_needed.
	 */
	PG_RETURN_INT32(pg_lolor_inv_drop(lobjId));
}
...
Datum
pg_lolor_loread(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
...
Datum
pg_lolor_lowrite(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
 ...
 */
Datum
pg_lolor_lo_import(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
pg_lolor_lo_import_with_oid(PG_FUNCTION_ARGS)
{
	text	   *filename = PG_GETARG_TEXT_PP(0);
...
lo_import_internal(text *filename, Oid lobjOid)
{
	 ...
	 */
	lo_cleanup_needed = true;
	oid = pg_lolor_inv_create(lobjOid);

	/*
	 * read in from the filesystem and write to the inversion object
	 */
	lobj = pg_lolor_inv_open(oid, INV_WRITE, CurrentMemoryContext);

	while ((nbytes = read(fd, buf, BUFSIZE)) > 0)	/* Flawfinder: ignore */
	{
		tmp = pg_lolor_inv_write(lobj, buf, nbytes);
		Assert(tmp == nbytes);
	}
 ...
 */
Datum
pg_lolor_lo_export(PG_FUNCTION_ARGS)
{
	Oid			lobjId = PG_GETARG_OID(0);
	 ...
	 */
	lo_cleanup_needed = true;
	lobj = pg_lolor_inv_open(lobjId, INV_READ, CurrentMemoryContext);
	 ...
	 */
	text_to_cstring_buffer(filename, fnamebuf, sizeof(fnamebuf));
	oumask = umask(S_IWGRP | S_IWOTH);	/* nosemgrep *//* Flawfinder: ignore —
										 * matches core be-fsstubs.c
										 * intentionally */
	PG_TRY();
	{
	...
	PG_FINALLY();
	{
		umask(oumask);			/* nosemgrep *//* Flawfinder: ignore —
	 ...
	 */
	while ((nbytes = pg_lolor_inv_read(lobj, buf, BUFSIZE)) > 0)
	{
...
lo_truncate_internal(int32 fd, int64 len)
{
	...
	pg_lolor_inv_truncate(lobj, len);
}

Datum
pg_lolor_lo_truncate(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
	int32		len = PG_GETARG_INT32(1);
...
Datum
pg_lolor_lo_truncate64(PG_FUNCTION_ARGS)
{
	int32		fd = PG_GETARG_INT32(0);
 ...
 */
void
AtEOXact_PG_LOLOR_LargeObject(bool isCommit)
{
	int			i;
	...
	pg_lolor_close_lo_relation(isCommit);
 ...
 */
void
AtEOSubXact_PG_LOLOR_LargeObject(bool isCommit, SubTransactionId mySubid,
...
lo_get_fragment_internal(Oid loOid, int64 offset, int32 nbytes)
{
	...
	lo_cleanup_needed = true;
	loDesc = pg_lolor_inv_open(loOid, INV_READ, CurrentMemoryContext);

	/*
	 * Compute number of bytes we'll actually read, accommodating nbytes == -1
	 * and reads beyond the end of the LO.
	 */
	loSize = pg_lolor_inv_seek(loDesc, 0, SEEK_END);
	if (loSize > offset)
	{
	...
	pg_lolor_inv_seek(loDesc, offset, SEEK_SET);
	total_read = pg_lolor_inv_read(loDesc, VARDATA(result), result_length);
	Assert(total_read == result_length);
	SET_VARSIZE(result, result_length + VARHDRSZ);

	pg_lolor_inv_close(loDesc);
 ...
 */
Datum
pg_lolor_lo_get(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
pg_lolor_lo_get_fragment(PG_FUNCTION_ARGS)
{
	Oid			loOid = PG_GETARG_OID(0);
	int64		offset = PG_GETARG_INT64(1);
 ...
 */
Datum
pg_lolor_lo_from_bytea(PG_FUNCTION_ARGS)
{
	Oid			loOid = PG_GETARG_OID(0);
	...
	lo_cleanup_needed = true;
	loOid = pg_lolor_inv_create(loOid);
	loDesc = pg_lolor_inv_open(loOid, INV_WRITE, CurrentMemoryContext);
	written = pg_lolor_inv_write(loDesc, VARDATA_ANY(str), VARSIZE_ANY_EXHDR(str));
 ...
 */
Datum
pg_lolor_lo_put(PG_FUNCTION_ARGS)
{
	Oid			loOid = PG_GETARG_OID(0);
	int64		offset = PG_GETARG_INT64(1);
	...
	/* Permission check */
	if (!lo_compat_privileges &&
		pg_lolor_largeobject_aclcheck_snapshot(loDesc->id,
											   GetUserId(),
											   ACL_UPDATE,
...
static bool
pg_lolor_object_ownercheck(Oid classid, Oid objectid, Oid roleid)
{
	Oid			ownerId;
	Relation	rel;
	ScanKeyData entry[1];
 ...
 */
AclResult
pg_lolor_largeobject_aclcheck_snapshot(Oid lobj_oid, Oid roleid, AclMode mode,
									   Snapshot snapshot)
{
	if (pg_lolor_largeobject_aclmask_snapshot(lobj_oid, roleid, mode,
 ...
 */
static AclMode
pg_lolor_largeobject_aclmask_snapshot(Oid lobj_oid, Oid roleid,
