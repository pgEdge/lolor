 ...
 * memory context given to pg_lolor_inv_open (for LargeObjectDesc structs).
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  contrib/pg_lolor/pg_lolor_inv_api.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
open_lo_relation(void)
{
	...
	if (lo_heap_r == NULL)
		lo_heap_r = table_open(get_PG_LOLOR_LargeObjectRelationId(), RowExclusiveLock);
	if (lo_index_r == NULL)
		lo_index_r = index_open(get_PG_LOLOR_LargeObjectLOidPNIndexId(), RowExclusiveLock);
 ...
 */
void
pg_lolor_close_lo_relation(bool isCommit)
{
	if (lo_heap_r || lo_index_r)
	{
		/*
...
/*
 * Same as pg_largeobject.c's PG_LOLOR_LargeObjectExists(), except snapshot to
...
/*
 *	pg_lolor_inv_create -- create a new large object
 *
 *	Arguments:
 ...
 */
Oid
pg_lolor_inv_create(Oid lobjId)
{
	Oid			lobjId_new;
	 ...
	 */
	lobjId_new = PG_LOLOR_LargeObjectCreate(lobjId);

	/*
	 * No shared dependency is recorded for the owner.  Core records one under
	 * classId LargeObjectRelationId, a genuine catalog the dependency
	 * machinery knows how to describe; an object in pg_lolor storage is a row
	 * in an ordinary table, and passing the OID of lolor.pg_largeobject
	 * produced pg_shdepend rows that DROP ROLE could not interpret, failing
	 * with "unrecognized object class" and leaving the role undroppable. They
	 * were never removed either, because inv_drop() deleted with
	 * PERFORM_DELETION_SKIP_ORIGINAL.  pg_shdepend is a shared catalog, so
	 * the stored classId was a per-database relation OID meaningless
	 * elsewhere.
	 *
...
/*
 *	pg_lolor_inv_open -- access an existing large object.
 *
 ...
 */
LargeObjectDesc *
pg_lolor_inv_open(Oid lobjId, int flags, MemoryContext mcxt)
{
	...
	/*
	 * Can't use PG_LOLOR_LargeObjectExists here because we need to specify
	 * snapshot
	 */
	...
	if ((descflags & IFS_RDLOCK) != 0)
	{
		if (!lo_compat_privileges &&
			pg_lolor_largeobject_aclcheck_snapshot(lobjId,
												   GetUserId(),
												   ACL_SELECT,
...
/*
 * Closes a large object descriptor previously made by pg_lolor_inv_open(), and
 * releases the long-term memory used by it.
 */
void
pg_lolor_inv_close(LargeObjectDesc *obj_desc)
{
	Assert(obj_desc);
	pfree(obj_desc);
}
 ...
 */
int
pg_lolor_inv_drop(Oid lobjId)
{
	/*
	 * There are no comments, security labels or dependencies to remove: an
	 * object in pg_lolor storage is not a catalog object, so nothing can be
	 * attached to it.  See the note in pg_lolor_inv_create().  The
	 * performDeletion() call that stood here searched pg_depend under a
	 * classId that is an ordinary relation OID and could never match.
	 */
	PG_LOLOR_LargeObjectDrop(lobjId);
...
int64
pg_lolor_inv_seek(LargeObjectDesc *obj_desc, int64 offset, int whence)
{
	int64		newoffset;
...
int64
pg_lolor_inv_tell(LargeObjectDesc *obj_desc)
{
	Assert(obj_desc);
...
int
pg_lolor_inv_read(LargeObjectDesc *obj_desc, char *buf, int nbytes)
{
	int			nread = 0;
	int64		n;
	int64		off;
...
int
pg_lolor_inv_write(LargeObjectDesc *obj_desc, const char *buf, int nbytes)
{
	int			nwritten = 0;
	int			n;
	int			off;
	...
	while (nwritten < nbytes)
	{
		...
		if (olddata != NULL && olddata->pageno == pageno)
		{
			...
			Assert(len >= 0 && len <= LOBLKSIZE);	/* validated by
													 * getdatafield() */
			...
			Assert(n > 0 && off + n <= LOBLKSIZE);	/* destination fits in
													 * workbuf page */
...
void
pg_lolor_inv_truncate(LargeObjectDesc *obj_desc, int64 len)
{
	...
	if (olddata != NULL && olddata->pageno == pageno)
	{
		...
		Assert(pagelen >= 0 && pagelen <= LOBLKSIZE);	/* validated by
														 * getdatafield() */
