...
# along with the pg_lolor tables and pg_catalog is left without a working
# lo_open().
...
is( $node->safe_psql(
		'postgres',
		"SELECT count(*) FROM pg_extension WHERE extname = 'pg_lolor'"),
	'0',
...
# objects are about to be dropped.  pg_lolor is not trusted, hence SUPERUSER.
$node->safe_psql('postgres',
	"CREATE ROLE regress_lolor_ext_owner SUPERUSER LOGIN");
$node->safe_psql(
	'postgres',
	"CREATE EXTENSION pg_lolor",
	extra_params => [ '-U', 'regress_lolor_ext_owner' ]);

my $owned = $node->safe_psql('postgres',
	"SELECT lo_from_bytea(0, 'rescued from drop owned')");

$node->safe_psql('postgres', "DROP OWNED BY regress_lolor_ext_owner");
